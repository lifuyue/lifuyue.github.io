...
export function Hero() {
  ...
  const obstacleRefs = useMemo(() => [copyRef, statsRef], []);
  ...
  const sceneScale = useTransform(progress, [0, 1], [1, 1.2]);
  const sceneX = useTransform(progress, [0, 1], ['0%', '7%']);
  ...
  const agentsOpacity = useTransform(progress, [0, 0.58, 0.9], [1, 0.9, 0]);
  ...
  return (
    ...
    >
      <div className="relative min-h-screen overflow-hidden pt-24 lg:sticky lg:top-0 lg:h-screen">
        <div aria-hidden="true" className="absolute inset-0 bg-noise" />
        <motion.div
          aria-hidden="true"
          style={prefersReducedMotion || !isDesktop ? undefined : { scale: sceneScale, x: sceneX }}
          className="absolute inset-0 opacity-95 will-change-transform"
        >
        ...
        <div aria-hidden="true" className="cinematic-grid absolute inset-0 opacity-55" />
        ...
        <motion.div
          aria-hidden="true"
          style={prefersReducedMotion || !isDesktop ? undefined : { scale: frameScale, opacity: frameOpacity }}
        ...
        <div className="section-shell relative z-20 min-h-[calc(100vh-6rem)]">
          {isDesktop ? (
            <motion.div
              style={prefersReducedMotion ? undefined : { opacity: agentsOpacity }}
              className="absolute inset-0"
            >
              <FloatingAgentCluster obstacleRefs={obstacleRefs} />
            </motion.div>
          ) : null}
