...
export function WorkArtwork({ kind }: { kind: string }) {
  if (kind === 'journal') return <div className="work-art art-journal" aria-hidden="true"><div className="art-label"><span>FIELD NOTES</span><span>VOL. 01</span></div><div className="journal-glyph">思<span>↗</span></div><div className="art-label"><span>HUMAN × AGENT</span><span>A YEAR IN THE MAKING</span></div></div>;
  ...
  return <div className="work-art art-suanxian" aria-hidden="true"><div className="art-label"><span>SUANXIAN / FRESHFLOW</span><span>2026</span></div><div className="fresh-symbol"><span /><span /><span /></div><strong>Fresh ideas.<br />Full delivery.</strong><div className="art-label"><span>报价 → 下单 → 配送 → 结算</span><span>↗</span></div></div>;
}
...
export function CinematicRail() {
  ...
  return (
    <section id="selected-work" className="reset-section selected-section">
      ...
      <div className="selected-heading"><h2>Selected<span className="serif-star">↗</span><br />perspectives<span className="work-count">({String(entries.length).padStart(2, '0')})</span></h2><p>Longform thinking.<br />Shipped systems.<br /><span>One continuous practice.</span></p></div>
      <div className="work-index">
        <Link to={entry.to} className="work-stage" aria-label={`查看：${entry.title}`}>
          ...
          <span className="preview-circle">Explore<br />↗</span>
        </Link>
        <div className="work-rows">
          {entries.map((item, index) => <Link key={item.id} to={item.to} className={`work-row ${active === index ? 'is-active' : ''}`} onPointerEnter={() => setActive(index)} onFocus={() => setActive(index)}>
            <div className="work-row-meta"><span>0{index + 1}</span><span>{item.type}</span><span className="work-row-arrow">↗</span></div>
      ...
      <div className="selected-bottom"><span>From thinking out loud to putting things out there.</span><div><Link className="text-link" to="/works">All works ↗</Link><Link className="text-link" to="/blog">The journal ↗</Link></div></div>
    </section>
  );
}
