...
export function BlogCard({ post }: { post: BlogPostEntry }) {
  return <article className="journal-entry"><Link to={`/blog/${post.slug}`}>
    ...
    <h3>{post.title}</h3><p>{post.description}</p><div className="entry-bottom"><div className="entry-tags">{post.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><span className="entry-arrow" aria-hidden="true">↗</span></div>
  </Link></article>;
}
