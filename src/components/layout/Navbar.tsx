...
export function Navbar() {
  ...
  return <>
    ...
    <header className={`reset-nav ${pathname === '/' ? 'reset-nav--home' : ''} ${scrolled ? 'nav-scrolled' : ''}`}>
      <Link to="/" className="reset-brand" aria-label="Lifuyue 首页"><span className="brand-mark">✳</span><span>Code by <strong>Lifuyue</strong></span></Link>
      <nav aria-label="主导航" className="reset-nav-links">{links.slice(1).map((link) => <Link key={link.to} to={link.to} aria-current={pathname === link.to ? 'page' : undefined}>{link.label}<span>↗</span></Link>)}</nav>
    </header>
    ...
    <dialog ref={dialog} id="site-navigation" className="navigation-dialog" aria-label="网站导航" data-lenis-prevent onClose={() => setOpen(false)} onClick={(event) => { if (event.target === event.currentTarget) close(); }}>
      <div className="navigation-sheet">
        ...
        <nav>{links.map((link, index) => <Link key={link.to} to={link.to} onClick={close} aria-current={pathname === link.to && !hash ? 'page' : undefined}><span className="menu-index">0{index + 1}</span><span>{link.label}</span><span className="menu-link-arrow">↗</span></Link>)}</nav>
        <div className="menu-footer"><span className="index-label">Elsewhere</span><div className="menu-socials">{socials.map((social) => <a key={social.label} href={social.href} target={social.href.startsWith('mailto:') ? undefined : '_blank'} rel="noreferrer">{social.label} ↗</a>)}</div>
